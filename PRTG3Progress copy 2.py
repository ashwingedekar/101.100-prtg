...
for id_value in tqdm(id_values, desc="Processing IDs"):  # Use tqdm for progress bar
    ...
    if response.status_code == 200:
        id_data = {
            ...
            "DISCARDS IN": None,   # New column for Discards In (Speed)
            "DISCARDS OUT": None,  # New column for Discards Out (Speed)
            "ERRORS IN": None,     # New column for Errors In (Speed)
            "ERRORS OUT": None     # New column for Errors Out (Speed)
        }
        ...
        try:
            ...
            if not selected_data.empty:
                ...
                id_data["DISCARDS IN"] = selected_data["Discards In (Speed)"].sum()
                id_data["DISCARDS OUT"] = selected_data["Discards Out (Speed)"].sum()
                id_data["ERRORS IN"] = selected_data["Errors In (Speed)"].sum()
                id_data["ERRORS OUT"] = selected_data["Errors Out (Speed)"].sum()
...
for data_dict in data_list:
    ...
    print(f"DISCARDS IN: {data_dict.get('DISCARDS IN', '< 0.01')}")    # Print Discards In (Speed)
    print(f"DISCARDS OUT: {data_dict.get('DISCARDS OUT', '< 0.01')}")  # Print Discards Out (Speed)
    print(f"ERRORS IN: {data_dict.get('ERRORS IN', '< 0.01')}")        # Print Errors In (Speed)
    print(f"ERRORS OUT: {data_dict.get('ERRORS OUT', '< 0.01')}")      # Print Errors Out (Speed)
    print("#" * 55)
