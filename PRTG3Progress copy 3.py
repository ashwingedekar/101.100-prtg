...
for id_value in id_values:
    ...
    if response_discard_in_upper_warning.status_code == 200:
        ...
        if match_discard_in_upper_warning:
            discard_in_upper_warning_limits[id_value] = int(match_discard_in_upper_warning.group(1)) * 8 / 1000000  # Convert bytes to megabits
    ...
    if response_discard_in_upper_error.status_code == 200:
        ...
        if match_discard_in_upper_error:
            discard_in_upper_error_limits[id_value] = int(match_discard_in_upper_error.group(1)) * 8 / 1000000  # Convert bytes to megabits
    ...
    if response_discard_in_lower_warning.status_code == 200:
        ...
        if match_discard_in_lower_warning:
            discard_in_lower_warning_limits[id_value] = int(match_discard_in_lower_warning.group(1)) * 8 / 1000000  # Convert bytes to megabits
    ...
    if response_discard_in_lower_error.status_code == 200:
        ...
        if match_discard_in_lower_error:
            discard_in_lower_error_limits[id_value] = int(match_discard_in_lower_error.group(1)) * 8 / 1000000  # Convert bytes to megabits
...
for id_value in tqdm(id_values, desc="Processing IDs"):  # Use tqdm for progress bar
    ...
    if response.status_code == 200:
        id_data = {
            ...
            "UPPER ERROR LIMIT": upper_error_limits.get(id_value, "not_set"),
            "LOWER ERROR LIMIT": lower_error_limits.get(id_value, "not_set"),
            "UPPER WARNING LIMIT": upper_warning_limits.get(id_value, "not_set"),
            "LOWER WARNING LIMIT": lower_warning_limits.get(id_value, "not_set")
        }
...
for data_dict in data_list:
   ...
   # print(f"THRESHOLD MESSAGE (MIN): {data_dict.get('THRESHOLD MESSAGE (MIN)', '< 0.01')}")
    ...
    print(f"UPPER ERROR LIMIT: {data_dict.get('UPPER ERROR LIMIT', 'not_set')}")
    print(f"LOWER ERROR LIMIT: {data_dict.get('LOWER ERROR LIMIT', 'not_set')}")
    print(f"UPPER WARNING LIMIT: {data_dict.get('UPPER WARNING LIMIT', 'not_set')}")
    print(f"LOWER WARNING LIMIT: {data_dict.get('LOWER WARNING LIMIT', 'not_set')}")
    print("#" * 55)
